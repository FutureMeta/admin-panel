...
/**
 * Un segnaposto: `%player%`, `%time%`. NON e' MiniMessage — lo sostituisce il
 * plugin prima di parlare col gioco — ed e' per questo che si riconosce a
 * parte e si disegna in un altro modo.
 */
export const PLACEHOLDER = /%[a-z0-9_]+%/gi;

/** Solo il tag, senza i codici legacy, e con `<!bold>` che spegne. */
const TAG_ONLY = /<\/?[a-zA-Z#!][^<>'"]*(?:(?:'[^']*'|"[^"]*")[^<>'"]*)*>/;
...
/**
 * I tag che il gioco conosce. Uno che non sta qui e' un errore, non un
 * segnaposto: in questi testi i segnaposto sono `%cosi%`, e un `<cosi>` che
 * il gioco non riconosce lo fa scartare tutto il messaggio.
 */
...
export type MiniMessageIssue = {
  /** Dove comincia, nel testo. */
  at: number;
  /** Il pezzo incriminato, com'e' scritto. */
  text: string;
  /** Perche', in una riga: e' quello che si legge sotto il campo. */
  reason: string;
};

/**
 * Cosa il gioco rifiuterebbe.
 *
 * NON E' «ogni tag va chiuso». In questi testi un colore si apre e basta —
 * `<gray>starts in <white>%time%` e' la forma normale — e MiniMessage lo
 * accetta. Rifiuta invece quattro cose, e sono queste:
 *
 *   1. un tag mai terminato: `<gradient:#8A8A8A:#CCCCCC Volver` — il `>` non
 *      arriva, e da li' in poi e' tutto tag;
 *   2. un nome che non conosce: `<gery>`;
 *   3. una chiusura senza apertura: `</bold>` con niente da chiudere;
 *   4. una sfumatura senza due colori validi.
 *
 * Il renderer (`renderMiniMessage`) e' PIU' TOLLERANTE di proposito: disegna
 * quello che puo' e ignora il resto, perche' chi scrive deve vedere il testo
 * anche a meta' frase. Questa funzione e' l'altra meta': dice se si puo'
 * salvare.
 */
export function validateMiniMessage(text: string): MiniMessageIssue[] {
  const issues: MiniMessageIssue[] = [];
  const stack: string[] = [];
  const tag = new RegExp(TAG_ONLY.source, 'y');

  for (let at = 0; at < text.length; at += 1) {
    if (text[at] !== '<') continue;
    const next = text[at + 1] ?? '';
    // Un `<` seguito da uno spazio, una cifra o niente e' testo: «a < b».
    ...
    if (m === null) {
      ...
      issues.push({
        at,
        text: text.slice(at, end === -1 ? text.length : end),
        reason: 'tag non chiuso: manca il `>`',
      });
      // Da qui a fine riga e' tutto dentro il tag rotto.
    ...
    const parsed = parseTag(raw);
    if (parsed === null) {
      issues.push({ at, text: raw, reason: 'tag vuoto' });
      at += raw.length - 1;
      continue;
    }

    // `<!bold>` spegne invece di accendere: e' lo stesso tag. Il nome si
    // risolve DOPO aver tolto il punto esclamativo, o `<!b>` non troverebbe
    // il suo alias.
    const bare = parsed.name.replace(/^!/, '');
    const name = ALIAS[bare] ?? bare;
    const isColour = colourOf(name) !== null;

    if (parsed.close) {
      const id = isColour ? 'color' : name;
      const i = stack.lastIndexOf(id);
      if (i === -1) issues.push({ at, text: raw, reason: 'chiusura senza apertura' });
      else stack.length = i;
    } else if (name === 'reset') {
      stack.length = 0;
    } else if (isColour) {
      ...
      if (colourOf(parsed.args[0] ?? '') === null) {
        issues.push({ at, text: raw, reason: 'colore sconosciuto' });
      } else stack.push('color');
    } else if (name === 'gradient') {
      if (stopsOf(parsed.args).length < 2) {
        issues.push({ at, text: raw, reason: 'una sfumatura vuole almeno due colori' });
      } else stack.push('gradient');
    } else if (KNOWN_TAGS.has(name)) {
      stack.push(name);
    } else {
      issues.push({ at, text: raw, reason: `tag sconosciuto: <${name}>` });
    }
