...
- **SEC-17** — Signed double-submit: cookie `__Host-metamc_csrf` (leggibile da JS, `SameSite=Strict`, `Secure`, `Max-Age` = `SESSION_ABSOLUTE_SECONDS`) contenente `base64url(HMAC-SHA256(k_csrf, session.id))`; il client lo rimanda in `X-CSRF-Token`; il server ricalcola dall'id di sessione e confronta con `timingSafeEqual`. Resistente al sottodominio che scrive cookie, a differenza del double-submit naive. La durata è quella della sessione e non è negoziabile: senza `Max-Age` il browser scarta il cookie alla chiusura mentre la sessione dura giorni, e con mezza coppia SEC-17 rifiuta ogni richiesta che cambia stato — compreso il login, che è l'unica con cui uscirne. Vedi `tests/acceptance/63-login-csrf-cookie.test.ts`.
