...
import { registerEmailRoutes } from './routes/account-email.ts';
import { registerPasswordRoutes } from './routes/account-password.ts';
...
import { registerDuelsConfigFileRoutes } from './routes/duels-config-files.ts';
...
import { registerUserAccessRoutes } from './routes/users-access.ts';
import { registerUserLifecycleRoutes } from './routes/users-lifecycle.ts';
...
export async function buildServer(ctx: AppContext): Promise<FastifyInstance> {
  ...
  await registerPasswordRoutes(app, ctx);
  await registerEmailRoutes(app, ctx);
  await registerInviteRoutes(app, ctx);
  await registerOnboardingRoutes(app, ctx);
  await registerUserRoutes(app, ctx);
  await registerUserAccessRoutes(app, ctx);
  await registerUserLifecycleRoutes(app, ctx);
