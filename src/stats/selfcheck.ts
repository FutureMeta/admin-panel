 ...
 *
 * L'ISTANTE E' UN PARAMETRO, non il `now()` del database. Con `now()` dentro
 * le query nessuno poteva fissare il momento del controllo: i test seminavano
 * dati relativi all'orologio vero e fallivano fra mezzanotte e l'una, quando
 * «l'ultima ora chiusa» e «oggi» cadono su due giorni diversi.
 */
function checksAt(now: Date): Check[] {
  return [
    {
      ...
      SELECT c.tick_at, c.players AS rete, coalesce(s.somma, 0) AS server
        ...
        LEFT JOIN (
          SELECT tick_at, sum(players)::bigint AS somma
           ...
           WHERE tick_at >= ${now}::timestamptz - interval '24 hours'
           GROUP BY 1
        ) s ON s.tick_at = c.tick_at
       WHERE c.tick_at >= ${now}::timestamptz - interval '24 hours'
         AND c.status = 'ok'
    ...
    },
    {
      ...
      SELECT s.tick_at, s.server_id, s.delta_s AS server, c.delta_s AS ciclo
       ...
       WHERE s.tick_at >= ${now}::timestamptz - interval '24 hours'
    ...
    },
    {
      ...
      SELECT bucket,
       ...
       WHERE bucket >= ${now}::timestamptz - interval '24 hours'
         AND bucket <  date_bin('5 minutes', ${now}::timestamptz, 'epoch'::timestamptz)
       GROUP BY bucket
      ...
      SELECT 'giorno' AS livello, d.day::text AS quando, d.server_id,
        ...
        JOIN (
          SELECT stats.civil_day(bucket) AS day, server_id, max(players_max) AS massimo
           ...
           WHERE bucket >= ${now}::timestamptz - interval '30 days'
           GROUP BY 1, 2
        ) h ON h.day = d.day AND h.server_id = d.server_id
       WHERE d.day >= stats.civil_day(${now}::timestamptz - interval '30 days')
         AND d.day <  stats.civil_day(${now}::timestamptz)
      ...
      SELECT 'ora', to_char(x.bucket, 'YYYY-MM-DD HH24:MI'), x.server_id,
        ...
        JOIN (
          SELECT date_bin('1 hour', bucket, 'epoch'::timestamptz) AS h, server_id,
            ...
            FROM stats.rollup_5m
           WHERE bucket >= ${now}::timestamptz - interval '48 hours'
           GROUP BY 1, 2
        ) m ON m.h = x.bucket AND m.server_id = x.server_id
       WHERE x.bucket >= ${now}::timestamptz - interval '48 hours'
         AND x.bucket <  date_bin('1 hour', ${now}::timestamptz, 'epoch'::timestamptz)
    ...
    },
    {
      ...
      WITH ore AS (
        SELECT date_bin('1 hour', c.tick_at, 'epoch'::timestamptz) AS h,
          ...
          FROM stats.poll_cycle c
         WHERE c.tick_at >= ${now}::timestamptz - interval '26 hours' AND c.status = 'ok'
         GROUP BY 1
        UNION ALL
        SELECT date_bin('1 hour', s.tick_at, 'epoch'::timestamptz), s.server_id,
         ...
         WHERE s.tick_at >= ${now}::timestamptz - interval '26 hours'
         GROUP BY 1, 2
      )
      SELECT to_char(r.bucket, 'YYYY-MM-DD HH24:MI') AS ora, r.server_id,
       ...
       WHERE r.bucket >= ${now}::timestamptz - interval '26 hours'
         AND r.bucket <  date_bin('1 hour', ${now}::timestamptz, 'epoch'::timestamptz)
    ...
    },
    {
      ...
      slot AS (
        SELECT g AS atteso
          FROM cadenza,
               generate_series(
                 to_timestamp(floor(extract(epoch FROM ${now}::timestamptz - interval '24 hours') / cadenza.n) * cadenza.n),
                 -- L'ultimo slot si esclude: potrebbe non essere ancora
                 -- scoccato, e un allarme che compare e sparisce a ogni giro
                 -- insegna a ignorarlo.
                 to_timestamp(floor(extract(epoch FROM ${now}::timestamptz) / cadenza.n) * cadenza.n) - interval '1 second',
    ...
    `,
      ...
      SELECT count(*) FILTER (
             ...
             (SELECT count(DISTINCT run_id) FROM stats.poll_cycle
               WHERE tick_at >= ${now}::timestamptz - interval '24 hours') AS run_nelle_24h
        FROM slot s, cadenza
    `,
    },
    {
      ...
      WITH per_modalita AS (
        SELECT p.day, vm.mode_key, count(DISTINCT p.player_id)::bigint AS unici
         ...
         WHERE p.day >= stats.civil_day(${now}::timestamptz) - 7 AND p.day < stats.civil_day(${now}::timestamptz)
         GROUP BY 1, 2
      ),
      limiti AS (
    ...
    },
    {
      ...
      offenders: sql`
      WITH giorni AS (
        SELECT stats.civil_day(${now}::timestamptz) - 7 AS da, stats.civil_day(${now}::timestamptz) AS a
      ),
      anagrafica AS (
...
/** Nomi e conseguenze dei controlli: non dipendono dall'istante. */
const CATALOG = checksAt(new Date());
 ...
 */
export async function runSelfcheck(db: Database, now: Date = new Date()): Promise<CheckOutcome[]> {
  const out: CheckOutcome[] = [];

  for (const check of checksAt(now)) {
    const t0 = Date.now();
    let failures = 0;
...
export const CHECK_NAMES: readonly string[] = CATALOG.map((c) => c.name);

/** Cosa diventa falso quando `name` fallisce. Va nella riga di log. */
export function consequenceOf(name: string): string {
  return CATALOG.find((c) => c.name === name)?.consequence ?? 'invariante sconosciuto';
}
