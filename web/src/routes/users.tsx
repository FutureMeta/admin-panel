// Utenti, sulla struttura del prototipo: la tabella (barra di ricerca in
// testa, conteggio a destra, paginazione in fondo), il dettaglio di una
// persona e l'invito. La schermata dei ruoli sta in `roles.tsx`.
//
...
import { ApiError, api, type Me, type UserDetail, type UserRow } from '../lib/api.ts';
import { LEVEL_LABELS, MODULE_TOTAL } from '../lib/modules.ts';
            ...
            >
              <span style={{ textTransform: 'capitalize', fontSize: 12.5, color: 'var(--tx-secondary)' }}>
                ...
                <span style={{ color: 'var(--tx-muted)' }}>
                  {' · '}
                  {LEVEL_LABELS[effective] ?? effective}
                </span>
              </span>
              <FilterSelect
                ...
                options={LEVEL_LABELS.map((name, level) => ({ value: String(level), label: name }))}
              />
            </div>
          );
        })}
